...
export default function LineWaves({
  ...
  useEffect(() => {
    ...
    // Listen on the container's parent section so overlaid content
    // doesn't swallow the events before they reach the canvas.
    function handleMouseMove(e) {
      const rect = container.getBoundingClientRect();
      // Only react when the cursor is actually over the container area
      if (
    ...
    // Make the canvas non-interactive so pointer events pass through to
    // the content layers above, while we track the mouse on the window.
    gl.canvas.style.pointerEvents = 'none';
    container.appendChild(gl.canvas);

    if (enableMouseInteraction) {
      // Attach to window so overlaid z-index layers don't block events
    ...
    function update(time) {
      ...
      if (enableMouseInteraction) {
        // Faster lerp (0.12) = snappier, matches React Bits feel
