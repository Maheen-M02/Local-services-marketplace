-- ServifyX Database Schema (Simplified for Supabase)
...
CREATE TABLE bookings (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    booking_number VARCHAR(50) UNIQUE NOT NULL DEFAULT 'SX' || EXTRACT(EPOCH FROM NOW())::bigint || LPAD(FLOOR(RANDOM() * 1000)::text, 3, '0'),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
-- Allow service role to bypass RLS (for our API)
CREATE POLICY "Service role can do everything on users" ON users FOR ALL USING (current_setting('role') = 'service_role');
CREATE POLICY "Service role can do everything on services" ON services FOR ALL USING (current_setting('role') = 'service_role');
CREATE POLICY "Service role can do everything on providers" ON providers FOR ALL USING (current_setting('role') = 'service_role');
CREATE POLICY "Service role can do everything on bookings" ON bookings FOR ALL USING (current_setting('role') = 'service_role');
CREATE POLICY "Service role can do everything on reviews" ON reviews FOR ALL USING (current_setting('role') = 'service_role');